  ...
  <el-dialog :title="title" :visible.sync="dialogFormVisible" :show-close="false" width="30%" center>
    <el-form :model="form">
      ...
      <el-form-item label="姓名" :label-width="formLabelWidth">
        <el-input v-model="form.user" autocomplete="off" style="width: 300px;"></el-input>
      </el-form-item>
      <el-form-item label="管辖宿舍楼" :label-width="formLabelWidth">
        <el-select style="width: 300px;" v-model="form.dormitoryId" clearable placeholder="请选择">
          <el-option
      ...
      <p style="text-align: center;color:red">宿舍管理员默认密码为：000000</p>
    </el-form>
...
export default {
  ...
  components: {
    ...
    mounted () {
      ...
      //   //ajax请求
      //   this.$store.dispatch('selectdormitoryAdmin')
      //       this.tableData=this.$store.state.dormitoryAdminlist.data
      //       this.total=this.$store.state.dormitoryAdminlist.total
      // }else{
      //   console.log('使用缓存数据')
      // }
        this.axios("/admin/selectdormitoryAdmin?pageSize="+this.pageSize+'&pageNo='+this.pageNo+'&'+ (new Date()).getTime().toString()).then(res=>{
            console.log(res)
            console.log(res.data)//axios自动包装data属性 res.data
            this.tableData=res.data.data
            this.total=res.data.total
            this.dormitorydata=res.data.dormitorydata
        }).catch(err=>{
            console.log(err);
        })
    },
    ...
    methods: {
      handleClose(){
        ...
        if(this.type==1){
          ...
          url='/admin/updatedormitoryAdmin'
          params={
            user: this.form.user,
            username: this.form.username,
            dormitoryId: this.form.dormitoryId,
            introduce: this.form.introduce,
            dormitoryAdminId: this.form.dormitoryAdminId
          }
        }
        this.axios({
          method: 'post',
      ...
      handleEdit(index, row) {
        ...
        this.form.dormitoryAdminId=row.dormitoryAdminId
        this.type=2
        console.log(index, row);
      },
      handleDelete(index, row) {
        console.log(row)
        this.axios("/admin/deldormitoryAdmin?dormitoryAdminId="+row.dormitoryAdminId ).then(res=>{
            if(res.data.meta.flag){
              this.$message({
                message: `删除成功`,
                type: 'success'
      ...
      delAll() {
        ...
        this.multipleSelection.forEach(element => {
          ids+=element.dormitoryAdminId+','
        });
        ids=ids.substring(0,ids.lastIndexOf(','))
        console.log(ids)
        this.axios("/admin/deldormitoryAdmin?dormitoryAdminId="+ids).then(res=>{
            if(res.data.meta.flag){
              this.$message({
                message: `删除成功`,
                type: 'success'
