<template>
  ...
  <el-dialog :title="title" :visible.sync="dialogFormVisible" :show-close="false" width="30%" center>
    <el-form :model="form">
      <el-form-item label="教师帐号" :label-width="formLabelWidth">
      ...
      <el-form-item label="教师名称" :label-width="formLabelWidth">
        <el-input v-model="form.teacherName" autocomplete="off" style="width: 300px;"></el-input>
      </el-form-item>
      ...
      <p style="text-align: center;color:red">教师默认密码为：jiaoshi</p>
    </el-form>
...
export default {
  ...
  components: {
    ...
    mounted () {
      ...
      // if(this.$store.state.classlist){
      //   //ajax请求
      //   this.$store.dispatch('selectclass')
      //       this.tableData=this.$store.state.classlist.data
      //       this.total=this.$store.state.classlist.total
      // }else{
      //   console.log('使用缓存数据')
      // }
        this.axios("/admin/selectteacher?pageSize="+this.pageSize+'&pageNo='+this.pageNo+'&'+ (new Date()).getTime().toString()).then(res=>{
            console.log(res)
    ...
    methods: {
      ...
      handleDelete(index, row) {
        console.log(row)
        this.axios("/admin/delteacher?teacherId="+row.teacherId ).then(res=>{
            if(res.data.meta.flag){
              this.$message({
                message: `删除成功`,
                type: 'success'
      ...
      delAll() {
        ...
        this.multipleSelection.forEach(element => {
          ids+=element.teacherId+','
        });
        ids=ids.substring(0,ids.lastIndexOf(','))
        console.log(ids)
        this.axios("/admin/delteacher?teacherId="+ids).then(res=>{
            if(res.data.meta.flag){
              this.$message({
                message: `删除成功`,
                type: 'success'
